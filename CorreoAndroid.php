...
$sql="select * from usuario where mail='".$_GET['mail']."' and estado >= 1";
...
if($cont!=0)
{
    if(mail($_GET['mail'],"Nueva Clave",$message,$cabeceras)){

        $sql2="update usuario set clave='".md5($clave)."' where mail='".$_GET['mail']."'";
        mysqli_query(conexion(),$sql2);

        $val=array('estado'=>1);
        //estado 1 para enviado
        echo json_encode($val);
    }else{
    
       // echo"Correo NO enviado";
       $val=array('estado'=>2);//errorCorreo no enviado estado =2
       echo json_encode($val);
    ...
    // echo"Correo NO enviado estado =3";
